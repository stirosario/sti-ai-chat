...
<body>
  ...
  <div id="sti-chat-box" role="dialog" aria-label="Chat STI Rosario"
    ...
    <!-- Sound controls -->
    <div id="sti-sound-controls" style="position:absolute; top:12px; right:46px; display:flex; gap:8px; align-items:center;">
      <button id="sti-sound-toggle" title="Activar/Desactivar sonido" style="background:transparent; border:1px solid rgba(255,255,255,0.04); color:#e6f0f6; padding:6px 8px; border-radius:8px; cursor:pointer;">🔊</button>
      <button id="sti-sound-gear" title="Ajustes de sonido" style="background:transparent; border:1px solid rgba(255,255,255,0.04); color:#e6f0f6; padding:6px 8px; border-radius:8px; cursor:pointer;">⚙️</button>
    </div>
    <div id="sti-sound-panel" style="display:none; position:absolute; top:48px; right:12px; width:320px; background:linear-gradient(180deg,#042237,#07293a); border:1px solid rgba(255,255,255,0.04); padding:12px; border-radius:10px; box-shadow:0 8px 30px rgba(2,6,23,0.6); z-index:9999;">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;"><strong>Sonido del typing</strong><small style="opacity:.7">Preferencias</small></div>
      <div style="display:flex; gap:8px; align-items:center; margin-bottom:8px;"><label style="flex:1; font-size:13px; color:#cfeffb">Activar sonido</label><input id="sti-sound-enabled" type="checkbox" /></div>
      <div style="margin-bottom:8px;"><label style="font-size:13px; color:#cfeffb">Volumen</label><input id="sti-sound-volume" type="range" min="0" max="1" step="0.01" style="width:100%"/></div>
      <div style="margin-bottom:8px;"><label style="font-size:13px; color:#cfeffb">Cadencia (ms)</label><input id="sti-sound-interval" type="range" min="100" max="1000" step="10" style="width:100%"/></div>
      <div style="margin-bottom:8px;"><label style="font-size:13px; color:#cfeffb">Timbre (pitch)</label><input id="sti-sound-pitch" type="range" min="0.5" max="2" step="0.01" style="width:100%"/></div>
      <div style="display:flex; gap:8px; justify-content:flex-end;"><button id="sti-sound-save" class="btn-primary">Guardar</button><button id="sti-sound-cancel" class="btn-secondary">Cancelar</button></div>
    </div>
    ...
    <div class="sti-input-area">
      ...
      <button id="sti-send" type="button" aria-label="Enviar mensaje">
        <svg width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
          <circle cx="13" cy="13" r="13" fill="url(#metal-blue)"/>
          <defs>
            <linearGradient id="metal-blue" x1="0" y1="0" x2="26" y2="26" gradientUnits="userSpaceOnUse">
              <stop stop-color="#5cc8ff"/>
              <stop offset="1" stop-color="#009dff"/>
            </linearGradient>
          </defs>
          <path d="M9 13h6M15 13l-2-2m2 2l-2 2" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
      </button>
    </div>
  </div>
...
<script>
  function initSTIChat(){
  console.log('Chat script inicializado');
  // ====== API base ======
  // ========================================================
  ...
  // Generate a short sample buffer that sounds like a click (white noise burst with decay)
  ...
  function createSampleBuffer(){
    ...
    // Fill with filtered noise -> simple envelope
    for (let i=0;i<len;i++){
      const t = i/len;
      // white noise scaled
      let n = (Math.random()*2 - 1) * (1 - t);
      // slight high-pass-ish (differentiate)
      if (i>0) n = 0.6 * n + 0.4 * data[i-1];
      // envelope
  ...
  function playSampleSound(opts={volume:0.08, pitch:1}){
    const ctx = ensureAudioCtx(); if(!ctx) return;
    try{
      if (!_sampleBuffer) _sampleBuffer = createSampleBuffer();
      const g = ctx.createGain();
      g.gain.value = (typeof opts.volume === 'number') ? opts.volume : 0.08;
      const src = ctx.createBufferSource();
      src.buffer = _sampleBuffer;
      src.playbackRate.value = opts.pitch || 1;
      src.connect(g); g.connect(ctx.destination);
      src.start();
      // cleanup
      src.onended = () => { try{ src.disconnect(); g.disconnect(); }catch(e){} };
    }catch(e){ /* fallback to oscillator */
      try{ const octx = ensureAudioCtx(); if(!octx) return; const o = octx.createOscillator(); const gg = octx.createGain(); o.type='sine'; o.frequency.value = (opts.pitch || 1) * 800; gg.gain.value = opts.volume || 0.08; o.connect(gg); gg.connect(octx.destination); const now = octx.currentTime; gg.gain.setValueAtTime(gg.gain.value, now); o.start(now); o.stop(now + 0.09); }catch(err){}
    }
  }

  function startTypingSound(){
    if (_typingSoundInterval) return;
    // create context on user gesture if needed
    ensureAudioCtx();
    // immediate play then interval according to stored preference
    const enabled = localStorage.getItem('sti_sound_enabled') !== 'false';
    if (!enabled) return;
    const vol = parseFloat(localStorage.getItem('sti_sound_volume') || '0.08');
    const pitch = parseFloat(localStorage.getItem('sti_sound_pitch') || '1');
    const cadence = parseInt(localStorage.getItem('sti_sound_interval') || '420', 10);
    try{ playSampleSound({ volume: vol, pitch: pitch }); }catch(e){}
    _typingSoundInterval = setInterval(()=>{ const en = localStorage.getItem('sti_sound_enabled') !== 'false'; if(!en) return; playSampleSound({ volume: parseFloat(localStorage.getItem('sti_sound_volume')||'0.08'), pitch: parseFloat(localStorage.getItem('sti_sound_pitch')||'1') }); }, cadence);
  }

  function stopTypingSound(){ if (_typingSoundInterval){ clearInterval(_typingSoundInterval); _typingSoundInterval = null; } }
  ...
  function addTyping(){
    ...
    msgs.scrollTop = msgs.scrollHeight;
    // play subtle typing sound while typing indicator is visible
    startTypingSound();
  }
  function removeTyping(){ if(typingEl){ typingEl.remove(); typingEl=null; } stopTypingSound(); }

  // ====== Sugerencias rápidas (problemas típicos) ======
  const suggestedProblems = [
    'Mi compu no prende',
    'No tengo internet',
    'Mi compu está lenta',
    'Activación de software',
    'Teclado / mouse no funcionan',
    'No sale sonido'
  ];

  function renderSuggestedProblems(containerNode){
    if (!containerNode) return;
    // evitar duplicados
    if (containerNode.querySelector('.sti-suggestions')) return;
    const wrap = document.createElement('div');
    wrap.className = 'sti-suggestions';
    const label = document.createElement('div');
    label.className = 'sti-suggestions-label';
    label.textContent = '¿Querés probar con alguna de estas opciones?';
    wrap.appendChild(label);
    const list = document.createElement('div');
    list.className = 'sti-suggestions-list';
    suggestedProblems.slice(0,6).forEach(text => {
      const b = document.createElement('button');
      b.type = 'button';
      b.className = 'sti-suggestion-btn';
      b.textContent = text;
      b.addEventListener('click', ()=>{
        // mostrar como mensaje de usuario y enviar
        addMsg(text, 'user');
        sendMsg(text);
      });
      list.appendChild(b);
    ...
    // insert after bubble inside node
    const bubble = containerNode.querySelector('.sti-bubble') || containerNode;
 ...
 // ====== WhatsApp ticket ======
  ...
  function normalizeButtons(payload){
    ...
    // Map tokens legacy -> canonical, y labels legibles
    const TOKEN_MAP = {
    ...
    const LABEL_MAP = {
      ...
      'BTN_MORE_TESTS': 'Pruebas avanzadas',
    ...
    try {
      ...
      if (Array.isArray(payload) && payload.length) {
        payload.forEach(it => {
          if (typeof it === 'string') {
            const mapped = TOKEN_MAP[it] || it;
            const label = (LABEL_MAP[mapped] || mapped);
            const value = mapped;
            out.push({ label: label, value: value, text: label });
            ...
            // si el value es un token, normalizarlo
            const mappedVal = TOKEN_MAP[value] || value;
            const finalLabel = LABEL_MAP[mappedVal] || label;
            out.push({ label: finalLabel, value: mappedVal, text: finalLabel, icon, description, example });
          }
        });
        // deduplicar por value
        const seen = new Set();
        const uniq = [];
        out.forEach(o => { if (!seen.has(o.value)) { seen.add(o.value); uniq.push(o); } });
        console.log('✅ Parsed as direct array:', uniq);
        return uniq;
      }
          ...
          if (Array.isArray(list)) {
            list.forEach(it => {
              if (typeof it === 'string') {
                const mapped = TOKEN_MAP[it] || it;
                const label = LABEL_MAP[mapped] || mapped;
                out.push({ label: label, value: mapped, text: label });
              } else if (it && it.label) {
                const value = it.value || it.label;
                const mapped = TOKEN_MAP[value] || value;
                const label = LABEL_MAP[mapped] || it.label;
                out.push({ label: label, value: mapped });
              }
            });
          }
        }
  ...
  async function sendButton(value, label){
    ...
    try {
      ...
      if (!data || !data.reply) {
        ...
        if (value === 'BTN_SOLVED') showSolvedMessage();
        return;
      }
  ...
  async function stiShowChat(){
    console.log('stiShowChat() iniciada');
    console.log('box elemento:', box);
    if (!box){ 
      console.error('No se encontro el elemento sti-chat-box'); 
      alert('No se encontro el elemento sti-chat-box'); 
      return; 
    }
    console.log('Mostrando chat...');
    
    // Forzar visualización removiendo display:none y estableciendo flex
    ...
    console.log('   - computed display:', window.getComputedStyle(box).display);
    console.log('   - visibility:', window.getComputedStyle(box).visibility);
    console.log('   - opacity:', window.getComputedStyle(box).opacity);
    ...
    if (!greeted){
      try{
        ...
        if (btns.length) {
          ...
          // Si el backend no provee botones, renderizamos sugerencias típicas
          renderSuggestedProblems(node);
        }
      }catch(err){
  ...
  if (headerBtn) {
    headerBtn.addEventListener('click', (e)=>{ 
      console.log('Click en headerBtn detectado');
      e.preventDefault(); 
      stiShowChat(); 
    }, {passive:false});
    console.log('Listener registrado en headerBtn');
  } else {
    console.error('headerBtn no encontrado');
  }
  
  if (heroBtn) {
    heroBtn.addEventListener('click', (e)=>{ 
      console.log('Click en heroBtn detectado');
      e.preventDefault(); 
      stiShowChat(); 
    }, {passive:false});
    console.log('Listener registrado en heroBtn');
  } else {
    console.error('heroBtn no encontrado');
  }
  
  if (closeBtn) {
    closeBtn.addEventListener('click', (e)=>{ 
      console.log('Click en closeBtn detectado');
      e.preventDefault(); 
      stiHideChat(); 
    }, {passive:false});
    console.log('Listener registrado en closeBtn');
  } else {
    console.error('closeBtn no encontrado');
  }
  ...
  // Si la URL tiene asistencia en el hash, abrir chat
  if (location.hash.includes('asistencia')) stiShowChat();
  
  // Listener para cuando cambia el hash (cuando hacen click en el botón)
  ...
  // Exponer función globalmente para debug desde consola
  window.forceOpenChat = function() {
    console.log('forceOpenChat() llamada desde consola');
    const chatBox = document.getElementById('sti-chat-box');
    console.log('Elemento encontrado:', chatBox);
    if (chatBox) {
      chatBox.style.display = 'flex';
      chatBox.style.visibility = 'visible';
      chatBox.style.opacity = '1';
      chatBox.style.zIndex = '2147483647';
      console.log('Chat forzado a mostrarse');
      console.log('Computed display:', window.getComputedStyle(chatBox).display);
    } else {
      console.error('No se encontro el elemento del chat');
    }
  };
  
  console.log('Script de chat cargado completamente');
  console.log('Ejecuta window.forceOpenChat() en la consola para forzar apertura');
  
  // ====== PWA Installation (DESHABILITADO) ======
  // El bloque de instalación PWA se mantiene solo como comentario histórico.
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initSTIChat, { once: true });
  } else {
    initSTIChat();
  }
</script>
