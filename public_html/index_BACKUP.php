...
<section id="por-que-elegir" class="accent accent-green">
  ...
  <div class="pqe-card accent accent-green">
    <ul class="pqe-list">
      ...
      <li>
        <span class="pqe-ico">
          <svg class="use-current" viewBox="0 0 24 24"><path d="M7 2v11h3v9l7-12h-4l4-8z"/></svg>
        </span>
      ...
      <li>
        <span class="pqe-ico">
          <svg class="use-current" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 5.58 2 10c0 2.97 2.11 5.55 5.19 7.03L6 22l5.27-3.2c.23.01.46.02.73.02 5.52 0 10-3.58 10-8s-4.48-8-10-8z"/></svg>
        </span>
      ...
      <li>
        <span class="pqe-ico">
          <svg class="use-current" viewBox="0 0 24 24"><path d="M12 2L2 22h20L12 2zm0 3.84L17.53 20H6.47L12 5.84z"/></svg>
        </span>
      ...
      <li>
        <span class="pqe-ico">
          <svg class="use-current" viewBox="0 0 24 24"><path d="M20 8h-3V4H7v4H4v14h16V8zm-5 0H9V6h6v2z"/></svg>
        </span>
      ...
      <li>
        <span class="pqe-ico">
          <svg class="use-current" viewBox="0 0 24 24"><path d="M12 2a10 10 0 0 0-2 19.8V22h4v-.2A10 10 0 0 0 12 2zm1 16h-2v-2h2v2zm0-4h-2V7h2v7z"/></svg>
        </span>
      ...
      <li>
        <span class="pqe-ico">
          <svg class="use-current" viewBox="0 0 24 24"><path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm0 4.15L18 8v3c0 4.02-2.8 7.97-6 9.19C8.8 18.97 6 15.02 6 11V8l6-2.85z"/></svg>
        </span>
...
<script>
  ...
  <ul class="icons">
    <li>
      <a href="https://www.instagram.com/sti.rosario" target="_blank" rel="noopener noreferrer">
        <img src="img/iconsinstagram.png" alt="Instagram STI" width="40" height="40" decoding="async">
      </a>
    </li>
    <li>
      <a href="https://maps.app.goo.gl/yfswNXVhSyYL1rgk9" target="_blank" rel="noopener noreferrer">
        <img src="img/iconslocation.png" alt="Ubicación STI" width="40" height="40" decoding="async">
      </a>
    </li>
    <li>
      <a href="mailto:sti.rosario@gmail.com?subject=Consulta%20STI" target="_blank" rel="noopener noreferrer">
        <img src="img/iconsemail.png" alt="Email STI" width="40" height="40" decoding="async">
      </a>
    </li>
    <li>
      <a href="https://wa.me/5493417422422?text=🔧%20STI%20Servicio%20Técnico%20Inteligente%0A💬%20¡Hola!%20👋%0A🌐%20Ingresé%20desde%20la%20web"
         target="_blank" rel="noopener noreferrer">
        <img src="img/iconswhatsapp.png" alt="WhatsApp STI" width="40" height="40" decoding="async">
      </a>
    </li>
    <li>
      <a href="https://www.mercadopago.com.ar/" target="_blank" rel="noopener noreferrer">
        <img src="img/iconsMP.png" alt="Mercado Pago" width="40" height="40" decoding="async">
      </a>
    </li>
    <li>
      <a href="https://www.facebook.com/stirosario" target="_blank" rel="noopener noreferrer">
        <img src="img/Face70.png" alt="Facebook STI" width="40" height="40" decoding="async">
      </a>
    </li>
  </ul>
</div>
...
<!-- === Script chat STI (inline, usando tus IDs) === -->
<script>
document.addEventListener('DOMContentLoaded', () => {
  // ⚙️ Endpoints
  ...
  function formatAssistant(txt){
    ...
    txt = String(txt)
      ...
      .replace(/\\n/g, '\n')
      .replace(/\n/g, '<br>')
      .replace(/https?:\/\/wa\.me\/\d+[^\s<]*/g, m => `<a href="${m}" target="_blank" rel="noopener">${m}</a>`);
    return txt;
  }

  async function stiExportToWhatsApp(btn){
    try {
      if (btn) btn.disabled = true;
      const res  = await fetch(`${API_BASE}/api/export/whatsapp`);
      const data = await res.json();
      const link = (data && data.ok && data.wa_link) ? data.wa_link : "https://wa.me/5493417422422";
  ...
  function stiRenderExportButton(container){
    ...
    b.onclick = () => stiExportToWhatsApp(b);
    container.appendChild(b);
  }

  function enhanceWhatsAppLinks(msgNode){
    if (!msgNode) return;
    const links = msgNode.querySelectorAll('a[href^="https://wa.me/"]');
    if (!links.length) return;
    links.forEach(a => {
      const wrap = document.createElement("div");
      stiRenderExportButton(wrap);
      a.replaceWith(wrap);
    });
  }
  ...
  function addMsg(text, who='bot'){
    ...
    // Botón de export a WA (no durante saludo)
    if (who === 'bot' && greeted && d.innerHTML.includes('https://wa.me/')) {
      enhanceWhatsAppLinks(d);
    }
  }
  ...
  async function showChat(){
    ...
    if(!greeted){
      try{
        ...
        const saludo = data?.greeting || '👋 ¡Hola! Soy Tecnos de STI. ¿Cómo te llamás?';
        addMsg(saludo, 'bot');
      }catch(e){
  ...
  // ID de sesión simple para mantener contexto servidor↔web
  const SESSION_ID = 'web-' + Math.random().toString(36).slice(2, 10);

// ⬇️ habilita el botón de export cuando responde el bot
function afterAddMsgHook(d, who) {
  // Evita insertar botón durante el saludo inicial
  if (who === 'bot' && !greeted) return;

  // Solo agrega el botón si el mensaje del bot contiene un enlace wa.me
  if (who === 'bot' && typeof enhanceWhatsAppLinks === 'function') {
    const hasWaLink = d.innerHTML.includes('https://wa.me/');
    if (hasWaLink) enhanceWhatsAppLinks(d);
  }
}
  ...
  async function showChat(){
    ...
    if(!greeted){
      try {
        // 👇 Trae el saludo formateado (con <br>, <b>, etc.) desde el JSON del servidor
        const r = await fetch(API_BASE + '/api/greeting', { method: 'GET' });
        const data = await r.json();
        const saludo = data?.greeting || '👋 ¡Hola! Soy Tecnos de STI. ¿Cómo te llamás?';
        addMsg(saludo, 'bot'); // muestra el mensaje de bienvenida dinámico
      } catch (e) {
        // fallback local por si el endpoint no responde
        addMsg('👋 ¡Hola! Soy Tecnos de STI. ¿Cómo te llamás?', 'bot');
      }
      greeted = true;
    }
  ...
  if (closeBtn)   closeBtn.addEventListener('click', hideChat, { passive: true });
  if (headerBtn)  headerBtn.addEventListener('click', (e)=>{ e.preventDefault(); showChat(); });
  if (heroBtn)    heroBtn.addEventListener('click',   (e)=>{ e.preventDefault(); showChat(); });
  ...
  async function sendMessage(){
    ...
    try {
      const r = await fetch(API_URL, {
        method:"POST",
        headers:{ "Content-Type":"application/json" },
        // 👇 el backend nuevo espera { text, sessionId }
        body: JSON.stringify({ text, sessionId: SESSION_ID })
      });
      const data  = await r.json();
      const reply = data?.reply || "No pude procesar la consulta.";
  ...
  // Si tu addMsg original no llama a afterAddMsgHook, podés integrarlo así:
  // function addMsg(text, who='bot'){
  //   const d = document.createElement('div');
  //   d.className = (who==='bot') ? 'msg bot' : 'msg user';
  //   d.innerHTML = text;  // ← importante: innerHTML para soportar <br> y <b>
  //   msgs.appendChild(d);
  //   afterAddMsgHook(d, who);
  // }
</script>
  ...
  <script>
  // Ejecuta cuando el navegador está libre (no bloquea la interacción)
  (window.requestIdleCallback || function(cb){return setTimeout(cb,1)})(function(){
    function track(eventName, params){ try{ gtag('event', eventName, params||{}); }catch(e){} }

    // WhatsApp (todos los enlaces a wa.me) + UTM si falta
    document.querySelectorAll('a[href*="wa.me/"]').forEach(a=>{
      try{
        ...
        if(!url.searchParams.get('utm_source')){
          ...
          url.searchParams.set('utm_campaign','cta_global');
          a.href = url.toString();
        }
      }catch(e){}
      a.addEventListener('click', ()=>track('click_whatsapp', {location: window.location.pathname}), {passive:true});
    });

    // AnyDesk
    const anydesk = document.querySelector('.boton-anydesk a');
    if(anydesk){
      anydesk.addEventListener('click', ()=>track('click_anydesk', {file:'AnyDesk.exe'}), {passive:true});
    }

    // Botón "Presupuesto sin cargo" (sección ¿Por qué elegir?)
    const presupuestoBtn = document.querySelector('.pqe-cta .btn-metal');
    if(presupuestoBtn){
      presupuestoBtn.addEventListener('click', ()=>track('click_presupuesto', {section:'por-que-elegir'}), {passive:true});
    }

    // Apertura de Chat STI (header y hero)
    const chatHeaderBtn = document.getElementById('btn-asistencia-header');
    if(chatHeaderBtn){
      chatHeaderBtn.addEventListener('click', ()=>track('open_chat_sti', {source:'header'}), {passive:true});
    }
    const chatHeroBtn = document.getElementById('btn-asistencia-hero');
    if(chatHeroBtn){
      chatHeroBtn.addEventListener('click', ()=>track('open_chat_sti', {source:'hero'}), {passive:true});
    }
  });
</script>
...
<script>
  ...
  // Mostrar solo si el usuario no aceptó antes
  if (!localStorage.getItem('stiCookiesAccepted')) {
    stiBanner.style.display = 'flex';
  }

  stiBtn.onclick = () => {
    localStorage.setItem('stiCookiesAccepted', 'true');
    stiBanner.style.display = 'none';
  };
</script>
...
<style>
  ...
  .cookie-banner {
    ...
    line-height: 1.5;
    padding: 14px 20px;
    border-top: 1px solid rgba(92,200,255,.25);
    box-shadow: 0 -2px 14px rgba(92,200,255,.15);
  ...
  .cookie-banner button {
    ...
    box-shadow: 0 0 10px rgba(92,200,255,.25);
  }
